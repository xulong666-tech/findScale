...
void 
DrawRotaRects(cv::Mat& img, std::vector<cv::RotatedRect>& box, cv::Scalar scalar= cv::Scalar(255, 0, 255));
cv::Mat
...
std::vector<cv::RotatedRect>
NMS2(const cv::Mat& img, std::vector<cv::RotatedRect>& boxSet, int simThres, bool isdebug);
