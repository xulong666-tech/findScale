...
float GetRRect2RRectDistance(const cv::RotatedRect& rrectA, const cv::RotatedRect& rrectB)
{
	Point2f ptsA[4],ptsB[4];//外接矩形四个端点的集合
	float dis_min = 0;
	rrectA.points(ptsA);
	rrectB.points(ptsB);
	dis_min = getDistance(ptsA[0], ptsB[0]);//赋予初始值
	for(Point2f ptA : ptsA)
		for (Point2f ptB : ptsB)
		{
			float dis_tmp = getDistance(ptA, ptB);
			if (dis_min > dis_tmp)
			{
				dis_min = dis_tmp;
			}
...
{
	Mat img_hsv;
	Mat colorGroundTruth = Mat::zeros(input_img.size(), CV_8UC3);
	//绿色
	...
	for (int h = 0; h < mask.rows; ++h)
	{
		for (int w = 0; w < mask.cols ; ++w)
		{
			...
			colorGroundTruth.at<Vec3b>(h, w)[2] = mask.at<uchar>(h, w);
	...
	//printf("i=%d", cnt);
	//imshow("hsv", colorGroundTruth);
	//imwrite("colorGroundTruth.bmp", colorGroundTruth);
	//waitKey(1);
	return cnt;
}
...
void DrawRotaRects(Mat &img, vector<RotatedRect> &box, Scalar scalar)
{
	...
	if (box[box.size() - 1].center.x > box[0].center.x)
	{
		...
		for (int i = 0; i < box.size(); i++)
		{
			...
			{
				line(img, Point(ps2[j2]), Point(ps2[(j2 + 1) % 4]), scalar, 2);
				//cv::putText(show,to_string(j), ps[j], 0, 1,Scalar(0, 0, 255), 2);  //将点集按顺序显示在图上
			}
			cv::putText(img, to_string(i), ps2[0], 0, 0.5, Scalar(0, 255, 255), 2);
		}
	}
	...
	else
	{
		...
		for (int i = 0; i < box.size(); i++)
		{
			...
			cv::putText(img, to_string(box.size() - 1 - i), ps2[0], 0, 0.5, Scalar(0, 255,255), 2);
...
cv::Mat
getRotaRectsMaskImg(cv::Mat& img, std::vector<cv::RotatedRect>& box, cv::Scalar scale)
{
	Point2f ps2[4];//外接矩形四个端点的集合
	Mat imgGT = Mat::zeros(img.size(), CV_8UC3);
	if (box.empty())
	{
		Error("box chains is empty!");
		return imgGT;
	...
	for (int i = 0; i < box.size(); i++)
	{
		...
		co_ordinates[0].push_back(ps2[0]);
		co_ordinates[0].push_back(ps2[1]);
		co_ordinates[0].push_back(ps2[2]);
		co_ordinates[0].push_back(ps2[3]);
		//图片区域绘制成白色的  
		drawContours(imgGT, co_ordinates, 0, scale, -1, 8);
...
int findProblScale(Mat img, vector<RotatedRect>& boxSet, int startThrs, int endThrs, int step, char* condition_argv[], bool isdebug)
{
	...
	if ((startThrs > 255 | startThrs < 0) | (endThrs>255 | endThrs < 0) | (step < 0 | step>255) | startThrs > endThrs)
	{
	...
	//Mat img_tmp;
	//cvtColor(img, img_tmp, CV_RGB2HSV);//转换为灰度图
	//vector<Mat> mv;
	//split(img_tmp, mv);
	//grayimg_ori = mv[0];
	//imshow("mv[0]", img_tmp);
	medianBlur(grayimg_ori, grayimg_ori, 3);//median filter
	//GaussianBlur(grayimg_ori, grayimg_ori, Size(3, 3),11,11);
	...
	for (int thrsValu = startThrs; thrsValu <= endThrs; thrsValu += step)
	{
		...
		//threshold(grayimg_ori, thre_img, thrsValu, 255, THRESH_BINARY);
		//Canny(grayimg_ori, thre_img, thrsValu, 180, 3);
		Canny(grayimg_ori, thre_img, thrsValu, thrsValu +50, 3);
		//Canny(grayimg_ori, thre_img, thrsValu, thrsValu+50, 3);
		if (isdebug)
		{
			//namedWindow("findthresimg", WINDOW_NORMAL);
			imshow("findthresimg", thre_img);
			imwrite("Img/ImgResult/Result" + to_string(thrsValu)+".png", thre_img);
			waitKey(1);
		}
		...
		Mat ImgFindedRect(show.rows, show.cols, show.type());
			...
			if ((areasize > areasize_min)&(areasize < areasize_max)&(rate_lw_min < rate_lw&rate_lw < rate_lw_max))//condition need to meet
			{
				...
				//if (isdebug)
				{
					for (int j = 0; j < 4; j++)
					{
						...
						line(ImgFindedRect, Point(ps[j]), Point(ps[(j + 1) % 4]), Scalar(255, 0, 255), 2);
					}
				}
			}
		...
		if (isdebug)
		{
			imwrite("Img/ImgResult/ImgFindedRect" + to_string(thrsValu) + ".png", ImgFindedRect);
...
vector<RotatedRect> NMS2(const Mat& img, vector<RotatedRect>& boxSet, int simThres, bool isdebug)
{
	Mat NMSimg = img.clone();
	vector<RotatedRect> retboxSet;//store the result needing to return.
...
vector <vector<RotatedRect>> PickSortscales(Mat img, vector<RotatedRect> &boxSet, bool isdebug, int sortMethod, int minChainNum, Rect view_Rect)
{
	...
	for (iter = boxSet1.end() - 1; !boxSet1.empty();)
	{
		...
		boxSet1.erase(iter);
		//find boxes in view,and find suitable box as neighbour scale.
		...
		sort(iterboxInView.begin(), iterboxInView.end(), [&](vector<RotatedRect>::iterator& iter1, vector<RotatedRect>::iterator& iter2) {
			...
			float dist1 = GetRRect2RRectDistance(rectx, (*iter1));
			float dist2 = GetRRect2RRectDistance(rectx, (*iter2));
			if (dist1 < dist2) return true;// from  small to big 
			else return false;
		});
		//Mat img_show = img.clone();
		////DrawRotaRects(img_show, iterboxInView);
		...
		for (i = 0; i < iterboxInView.size(); i++)
		{
			if (IsRRmeetPositionCondition(rectx, (*iterboxInView[i])))
			{
				if (isdebug)
				{
					...
					drawRotatedRect(imgPickSortscales, rectx,Scalar(255,60,0),2);
					circle(imgPickSortscales,rectx.center,2, Scalar(200, 60, 200),-1);
					...
					static int k = 0;
					imwrite("Img/ImgResult/PSscales_img" + to_string(k) + ".png", imgPickSortscales); k++;
					waitKey(1000);
					//waitKey(1);
				}
